 ...
 */
public class JenkinsBuildPage extends BasePage {    
            
    public JenkinsBuildPage() {        
        Item item = new Item();
        item.setSpace(getCurrentSpace());
        item.setStatus(State.NEW);        
    }
    
    public JenkinsBuildPage(String refId, String appReporsitoryName) {        
        ...
        try {
    		...
    		params.put("svn.build.type", "C");
    		params.put("svn.issue.number", refId);
    		params.put("svn.app.reporsitory.name", appReporsitoryName);
    		
			String buildString = cmd.build(jobName, params);
